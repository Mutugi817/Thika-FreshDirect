...
const ProductCard = ({product}) => {
        ...
        const {currency, navigate, addToCart, cartItems, removeFromCart} = useAppContext();

  return product && (
            <div  onClick={() => {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0, 0)}}
            className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-50 max-w-56 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                           ...
                           <img key={i} src={i < 4 ? assets.star_icon : assets.star_dull_icon} alt="star_icon" className='md:w-3.5 w3'/>
